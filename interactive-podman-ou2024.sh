...
podman-hpc run ${PODMAN_HPC_RUN_GPU} \
    ...
    --mount type=bind,source=/pscratch/sd/m/masao/roman_snpit/database_dirs_ou2024,target=/data \
    ...
    --mount type=bind,source=/global/cfs/cdirs/m4385/romanisim_sims,target=/romanisim_sims \
    ...
    --env TERM=xterm \
    --env SNPIT_DEFAULT_CONFIG=/snpit_env/configs/ou2024_container_config.yaml \
    --env SNPIT_CONFIG=/snpit_env/configs/ou2024_container_config.yaml \
    --annotation run.oci.keep_original_groups=1 \
    -it \
    registry.nersc.gov/m4385/roman-snpit-env:${WHICHROMANENV:-cpu}-0.1.47 \
    /bin/bash ${@}
